# Public multi-AZ ROSA HCP using modules from rh-mobb/validated-pattern-terraform-rosa.
# All three Git module sources MUST use the same ref string (Terraform does not allow variable interpolation in source).
# Pinned to 28b2933 — includes the d6ae385 fix for the CLUSTERS-MGMT-403 race (per-pool vs total replicas).
# AutoNode-specific settings (provision_shard_id, additional_cluster_properties) live in clusters/hcp-autonode/terraform/.
