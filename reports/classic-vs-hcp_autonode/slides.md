...
**Fast workload loop (HPA)** can outrun **slow cluster loop (CAS / AutoNode)**.

- **Low‑priority pause Pods** (**balloons**) **reserve CPU/memory requests** ahead of bursts — slack that disappears when production Pods preempt them.
- **Scheduler** preempts balloons first → new replicas **often schedule without** waiting for **node provisioning**.
- **Purpose:** deliberate **reserved headroom** bridging **seconds‑scale HPA decisions** and **minute‑scale** fleet provisioning
...
<div class="text-xs font-semibold mb-1" style="color: var(--rh-muted)">AUTONODE (HCP) — event-driven</div>

```mermaid
stateDiagram-v2
  direction LR
  classDef waiting fill:#C9190B,color:#fff,font-weight:bold
  classDef action fill:#3E8635,color:#fff,font-weight:bold
  [*] --> Pending: pod cannot be scheduled
  Pending --> NodeClaim: AutoNode reacts (~1s) — sizes instance to fit workload
  NodeClaim --> Launching: EC2 Fleet API call
  Launching --> Booting: instance running (~10s)
  Booting --> Ready: HCP worker bootstrap (~4m)
  Ready --> [*]: pod scheduled and Running
  class Pending waiting
  class NodeClaim action
  class Launching action
  class Booting action
  class Ready action
```

<div class="text-xs text-[var(--rh-muted)] mb-2">↑ Acts on <code>Pending</code> pods immediately — no need to wait for a scheduling failure event</div>

<div class="text-xs font-semibold mb-1" style="color: var(--rh-muted)">CAS (CLASSIC) — polling loop</div>
...
stateDiagram-v2
  ...
  classDef action fill:#3E8635,color:#fff,font-weight:bold
  [*] --> Pending: pod cannot be scheduled
  Pending --> FailedScheduling: kube-scheduler gives up
  FailedScheduling --> Polling: CAS loop evaluates (10–30s cadence)
  Polling --> MachineSet: TriggeredScaleUp — MachineSet replica +1
  MachineSet --> Booting: Machine API → EC2 RunInstances
  Booting --> Ready: three-boot sequence — Ignition → MCD → final (~6m)
  Ready --> [*]: pod scheduled and Running
  class Pending waiting
  class FailedScheduling waiting
  class Polling waiting
  class MachineSet action
  class Booting action
  class Ready action
```

<div class="text-xs text-[var(--rh-muted)] mt-1">↑ Waits for <code>FailedScheduling</code> then enters a polling loop — adds 10–30s before any action is taken</div>

<div class="mt-5 max-w-4xl mx-auto text-sm pl-4 border-l-4" style="border-color: var(--rh-red); color: var(--rh-muted);">

<strong style="color: var(--rh-text)">Insight —</strong> After bootstrap, EC2 is the heavy lift for both paths. Before that: <strong>AutoNode</strong> reacts to <strong><code>Pending</code></strong> pods and picks <strong>workload-shaped</strong> nodes. <strong>CAS</strong> waits for <strong><code>FailedScheduling</code></strong> and its <strong>poll interval</strong>, then grows a <strong>uniform pool-shaped</strong> node.
...
<!-- SLIDE 22 — Scale-down state diagrams (policy reference, not per-step observation) -->

# How Scale-Down Works

<div class="text-xs font-semibold mb-1" style="color: var(--rh-muted)">AUTONODE (HCP) — AutoNode reclaim path (<strong>schematic</strong>)</div>
...
stateDiagram-v2
  ...
  Evaluating --> Packing: pods can repack onto fewer nodes?
  Evaluating --> Watching: pods cannot repack — no action taken
  Packing --> Draining: cordon + evict pods to remaining nodes
  Draining --> Terminating: pods rescheduled
  Terminating --> [*]: EC2 instance terminated
  class Watching waiting
  class Evaluating action
  class Packing action
  class Draining action
  class Terminating action
```

<div class="text-xs text-[var(--rh-muted)] mb-2">↑ Benchmark clock: workload delete → <strong>NodeClaim drops</strong> → <strong>stable node count</strong> — not each schematic box on-screen.</div>

<div class="text-xs font-semibold mb-1" style="color: var(--rh-muted)">CAS (CLASSIC) — utilisation threshold (<strong>schematic</strong>)</div>
...
stateDiagram-v2
  ...
  classDef waiting fill:#C9190B,color:#fff,font-weight:bold
  classDef action fill:#3E8635,color:#fff,font-weight:bold
  [*] --> Monitoring: workload removed
  Monitoring --> Waiting: node falls below utilisation threshold
  Waiting --> Draining: scale-down-delay-after-add (10m default) elapsed
  Draining --> Terminating: pods evicted
  Terminating --> [*]: EC2 instance terminated
  class Monitoring waiting
  class Waiting waiting
  class Draining action
  class Terminating action
```

<div class="text-xs text-[var(--rh-muted)] mt-1">↑ Benchmark times <strong>cooldown elapsed → cordon → drain → node gone</strong>. The <strong>~10 m</strong> delay is mostly policy cooldown, not eviction work.</div>
...
<!--
Speaker note: Reference diagrams only — benchmark clock was workload dropped → fewer NodeClaims /
stable Ready count (AutoNode) vs cooldown elapsed → cordon → drain timing (CAS). Pack-first maths +
consolidateAfter debounce versus threshold scan + mandatory delays; neither path is wrong, different default.
-->
...
<!--
Speaker note: Increase in traffic causes HPA to schedule more pods, Balloon pods are descheduled, and cluster scales up to reschedule them, keeping spare capacity.  The effective difference between Classic and HCP w/ Autonode is minimal.
-->
...
<RhTwoColumn>
  ...
  - Phase 1 (no prep): **9m 48s** to pods Ready
  - Phase 2 (balloon pods): **31s**
  - **~95%** reduction
  ...
  - Phase 1 (no prep): **6m 34s**
  - Phase 2 (balloon pods): **1m 03s**
  - **~84%** reduction
